...
const quotes = [
  ...
  { r: "Клиент", q: "Смета не разъехалась, сроки выдержаны. Материалы и стыки — как на референсах премиум-брендов." },
  { r: "Партнёр", q: "SO Group — редкий кейс безупречной реализации: сроки, сметы и качество совпадают." },
  { r: "Клиент", q: "Деликатный сервис: тишина, порядок и уважение к пространству. Это роскошь." },
  { r: "Архитектор", q: "Команда слышит и дорабатывает детали. Работать с ними — эстетическое уд��вольствие." },
];
  ...
  return (
    <section id="testimonials" className="container py-[--section-gap]">
      <div className="flex items-end justify-between">
        ...
        <div className="text-sm text-muted-foreground">{quotes.length} отзывов</div>
      </div>
            ...
            {quotes.map((c, i) => (
              <CarouselItem key={i}>
                <article className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-soft">
                  ...
                  <div className="text-xs text-muted-foreground uppercase tracking-wider">{c.r}</div>
